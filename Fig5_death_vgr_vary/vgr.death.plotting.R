...
Standard   <- read.table("/Users/user/Downloads/fig_death_vgr_vary/my_fy_mating_summary.txt",sep=",",header=T)
normal <- read.table("/Users/user/Downloads/fig_death_vgr_vary/my_fn_mating_summary.txt",sep=",",header=T)
closed <- read.table("/Users/user/Downloads/fig_death_vgr_vary/mn_fn_mating_summary.txt",sep=",",header=T)
...
split2[split2 == "mn_fn"] <- "Closed"
split2[split2 == "my_fn"] <- "Normal"
split2[split2 == "my_fy"] <- "Standard"
...
#unmated
#grid.arrange(pop,unmated,nrow=2,newpage=T)
mean_survive <- c(mean(split2$p_survive[split2$role=="Closed"]),
mean(split2$p_survive[split2$role=="Normal"]),
mean(split2$p_survive[split2$role=="Standard"]))



mean_dikaryon <- c(
  mean(split2$p_dikaryon[split2$role=="Closed"],na.rm=T),
mean(split2$p_dikaryon[split2$role=="Normal"],na.rm=T),
mean(split2$p_dikaryon[split2$role=="Standard"],na.rm=T))
overview <- data.frame(mean_mating = mean_dikaryon,
                       mean_pop = mean_survive,
                       role = c("Closed","Standard","Standard"))
...
pdf("/Users/user/Downloads/fig_death_vgr_vary/fig_death_vgr.Apr26.pdf",width=10,height=10)
